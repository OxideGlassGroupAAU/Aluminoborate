...
# Startværdier
    for ind, i in enumerate(R_data, start=0):    
        ...
        ### Draw of all Al
        if Al5[-1] > 0: 
            while (Al5[-1]*3) > (Al4A[-1]+Q4A[-1]+Q2A[-1]+(Q1AA[-1]*2)+(Q0AAA[-1]*3)):
                gQ3 = (Q3[-1]*w[6]) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
                gQ4 = (Q4A[-1]*w[7]) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
                gQ2 = (Q2A[-1]*w[8]) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
                gQ1 = (Q1AA[-1]*w[9]) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
                gAl5 = (Al5[-1]*w[10]) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
...
#res = scipy.optimize.basinhopping(model, w1, niter=5, T=1.0, stepsize=0.5, minimizer_kwargs=None, take_step=None, accept_test=None, callback=print_SSE, interval=50, disp=True, niter_success=None)
#
...
#    
#    plt.subplot(131)
#    b3, b4, b2, b1, b0, a4, a5 = plt.plot(Q3_data, Q3_m[0:25],'ro', Q4_data, Q4_m[0:25] , 'ys', Q2_data, Q2_m[0:25], 'gd', Q1_data , Q1_m[0:25], 'cp', Q0_data, Q0_m[0:25], 'k*', Al4_data, Al4_m[0:25] , 'm^', Al5_data, Al5_m[0:25] , 'bv')
#    plt.plot(a ,fit_fn(a),':k')
#    plt.xlabel("Experimental (%)", fontproperties=font)
#    plt.ylabel("Model (%)", fontproperties=font)
#    #plt.legend(handles=[Q3_red, Q4_yellow, Q2_green, Al5_blue, Al4_magenta, Q1_cyan, Q0_black])
