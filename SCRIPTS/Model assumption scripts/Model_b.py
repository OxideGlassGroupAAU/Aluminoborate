...
Al_data = [float(i) for i in R_data]
...
# Model
def model(w1):
    w=np.array([1, abs(w1[0]), abs(w1[1]), abs(w1[2]), abs(w1[3]), abs(w1[4]), abs(w1[5])])
    
    SSE = []
    
# Startværdier
    for i in R_data:    
        Q3 = [(300*w[6]*i + 300*w[6]-400)/((3*w[6]-4)*i+3*w[6]-4), ]
        Q4 = [(-300*w[6]*i)/((3*w[6]-4)*i+3*w[6]-4), ]
        Q2 = [0, ]
        Q1 = [0, ]
        Q0 = [0, ]
        Al5 = [-100*(i/((3*w[6]-4)*i+3*w[6]-4)), ]
        Al4 = [(300*w[6]-300)*(i/((3*w[6]-4)*i+3*w[6]-4)), ]
    ...
    #For loop for udregning af p og Q
        for i in draw_ar:
            ...
            if M2O[i] < (100-M2O[i])*w[5]+(R_data[R_ind]/(1+2*R_data[R_ind]))*100:
                P = 1
            else:
                P = 0
            ...
            if Q2[-1] + pQ4 - pQ2 + pQ3*(1-P) - pQ4*pQ2 + pQ4*pQ3*(1-P) + pQ4**2 < 0:
                next_Q2 = 0
            else:
                next_Q2 = Q2[-1] + pQ4 - pQ2 + pQ3*(1-P) - pQ4*pQ2 + pQ4*pQ3*(1-P) + pQ4**2
            ...
            if Al5[-1] - 0.25*pAl5 - pQ4*0.25*pAl5 < 0:
                next_Al5 = 0
            else: 
                next_Al5 = Al5[-1] - 0.25*pAl5 - pQ4*0.25*pAl5
            
            if Al4[-1] + 0.25*pAl5 + pQ4*0.25*pAl5 < 0:
                next_Al4 = 0
            else: 
...
#w = [1, abs(res.x[0]), abs(res.x[1]), abs(res.x[2]), abs(res.x[3]), abs(res.x[4]), abs(res.x[5])]

w = [1,2.89890726467,5.71064681598e-11,19.2057890517,13.1841750131,0.327867590781,0.18021808235]
