...
mod_data = [float(i) for i in mod_data[0:25]]
Al_data = [float(i) for i in Al_data[0:25]]
R_data = [float(i) for i in R_data[0:25]]
Q3_data = [float(i) for i in Q3_data[0:25]]
Q4_data = [float(i) for i in Q4_data[0:25]]
Q2_data = [float(i) for i in Q2_data[0:25]]
Q1_data = [float(i) for i in Q1_data[0:25]]
Q0_data = [float(i) for i in Q0_data[0:25]]
Al5_data = [float(i) for i in Al5_data[0:25]]
Al4_data = [float(i) for i in Al4_data[0:25]]
...
# Model
def model(w, r_in = R_data):
    w=np.array([1, 0.03426035469186162, abs(w[1]), abs(w[2]), abs(w[3]), abs(w[4]), abs(w1[5]), abs(w1[6]), abs(w1[7]), abs(w1[8]), abs(w1[9]), abs(w[10])])
    iw = 1/w
    ...
    for ind, i in enumerate(r_in, start=0):    
        ...
        if Al5[-1] > 0: 
            while (Al_draw[-1]/3)*100/(100+(Al_draw[-1]/3)) < Al5[-1]*100/(100-Al4A[-1]):
                gQ3 = (Q3[-1]*w[6])*(1/(r_in[R_ind]+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
                gQ4 = (Q4A[-1]*w[7])*(1/(r_in[R_ind]+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
                gQ2 = (Q2A[-1]*w[8])*(1/(r_in[R_ind]+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
                gQ1 = (Q1AA[-1]*w[9])*(1/(r_in[R_ind]+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
                gAl5 = (Al5[-1]*w[10])*(1/(r_in[R_ind]+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
            ...
            gQ3 = balance_draw*(Q3[-2]*w[6])*(1/(r_in[R_ind]+1)) / (Q3[-2]*w[6]+Q4A[-2]*w[7]+Q2A[-2]*w[8]+Q1AA[-2]*w[9]+Al5[-2]*w[10])
            gQ4 = balance_draw*(Q4A[-2]*w[7])*(1/(r_in[R_ind]+1)) / (Q3[-2]*w[6]+Q4A[-2]*w[7]+Q2A[-2]*w[8]+Q1AA[-2]*w[9]+Al5[-2]*w[10])
            gQ2 = balance_draw*(Q2A[-2]*w[8])*(1/(r_in[R_ind]+1)) / (Q3[-2]*w[6]+Q4A[-2]*w[7]+Q2A[-2]*w[8]+Q1AA[-2]*w[9]+Al5[-2]*w[10])
            gQ1 = balance_draw*(Q1AA[-2]*w[9])*(1/(r_in[R_ind]+1)) / (Q3[-2]*w[6]+Q4A[-2]*w[7]+Q2A[-2]*w[8]+Q1AA[-2]*w[9]+Al5[-2]*w[10])
            gAl5 = balance_draw*(Al5[-2]*w[10])*(1/(r_in[R_ind]+1)) / (Q3[-2]*w[6]+Q4A[-2]*w[7]+Q2A[-2]*w[8]+Q1AA[-2]*w[9]+Al5[-2]*w[10])
...
#res = scipy.optimize.basinhopping(model, w1, niter=500, T=1.0, stepsize=0.5, minimizer_kwargs=None, take_step=None, accept_test=None, callback=print_SSE, interval=50, disp=True, niter_success=None)
#
...
with open("{}.csv".format(fil2), newline='') as csvfile:
    spamreader=csv.reader(csvfile,delimiter=';', quotechar="|")
    for column in spamreader:
        w1.append(column[0])
...
w = [abs(res.x[0]), abs(res.x[1]), abs(res.x[2]), abs(res.x[3]), abs(res.x[4]),  w1[5], w1[6],  w1[7],  w1[8],  w1[9], abs(res.x[10])]
...
model(w)
ssef = model(w)
print("Final SSE = {}".format(ssef))

t = list(range(100))
plt.plot(Q4_data, Q4_m, 'rd', Q3_data, Q3_m, 'bd', Q2_data, Q2_m, 'gd', Q1_data, Q1_m, 'yd', Al4_data, Al4_m, 'kd', 
         Al5_data, Al5_m, 'cd'
         )
plt.plot(t, t, 'k--')
...
plt.savefig(os.path.join(f'{fil}_data',f'{fil} Model_5.png'))
plt.show()

m_data_plt_kval = np.column_stack([Q3_data, Q3_m, Q4_data, Q4_m, Q2_data, Q2_m, Q1_data, Q1_m, Al4_data, Al4_m, Al5_data, Al5_m])
np.savetxt(os.path.join(f'{fil}_data', f"Model_Qn_v_data_Qn_{fil}.csv"), m_data_plt_kval)
np.savetxt(os.path.join(f'{fil}_data', f"Model_5_params.csv"), w)
...
w = [1, 0.03426035469186162, abs(res.x[1]), abs(res.x[2]), abs(res.x[3]), abs(res.x[4]),  w1[5], w1[6],  w1[7],  w1[8],  w1[9], abs(res.x[10])]

while (Al_draw[-1]/3)*100/(100+(Al_draw[-1]/3)) < Al5[-1]*100/(100-Al4A[-1]):
    gQ3 = (Q3[-1]*w[6])*(1/(r_Al+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
    gQ4 = (Q4A[-1]*w[7])*(1/(r_Al+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
    gQ2 = (Q2A[-1]*w[8])*(1/(r_Al+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
    gQ1 = (Q1AA[-1]*w[9])*(1/(r_Al+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
    gAl5 = (Al5[-1]*w[10])*(1/(r_Al+1)) / (Q3[-1]*w[6]+Q4A[-1]*w[7]+Q2A[-1]*w[8]+Q1AA[-1]*w[9]+Al5[-1]*w[10])
...
Alp = (((Q4A/3)+(Q2A/3)+(Q1AA*2/3)+Q0AAA+(Al4A*4/3))/((Q4A/3)+(Q2A/3)+(Q1AA*2/3)+Q0AAA+(Al4A*4/3)+Q3+Q4A+Q2A+Q1AA+Q0AAA))*100

plt.plot(Al_draw, Q3, 'r-', Al_draw, Q4A, 'b-', Al_draw, Q2A, 'g-', Al_draw, Q1AA, 'y-', Al_draw, Q0AAA, 'm-', Al_draw, Al4A, 'k-', 
         Al_draw, Al5, 'c-'
         )
#plt.plot(t, t, 'k--')
plt.axis([0, 450, 0, 30])
plt.xlabel("Al_draw")
plt.ylabel("Qn")
plt.title('Binary AlB')
#plt.savefig(os.path.join(f'{fil}_data',f'Binary_AlB.png'))
plt.show()

plt.plot(Alp, Q3, 'r-', Alp, Q4A, 'b-', Alp, Q2A, 'g-', Alp, Q1AA, 'y-', Alp, Q0AAA, 'm-', Alp, Al4A, 'k-', 
         Alp, Al5, 'c-'
         )
#plt.plot(t, t, 'k--')
plt.axis([0, 70, 0, 30])
...
AlB_dat = np.column_stack([Al_draw, Alp, Q3, Q4A, Q2A, Q1AA, Al4A, Al5,])
np.savetxt(os.path.join(f'{fil}_data', f"Binary_AlB_res.csv"), AlB_dat)
